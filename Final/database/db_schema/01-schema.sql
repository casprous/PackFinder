...
CREATE TABLE IF NOT EXISTS `Location` (
  ...
  `is_desk` TINYINT(1) NOT NULL DEFAULT 0,
  PRIMARY KEY (`loc_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
...
CREATE TABLE Notification (
    ...
    notif_usr_id INT NOT NULL, -- The user receiving the notification
    notif_item_id INT NOT NULL, -- The related post
    notif_type ENUM('reply', 'status') NOT NULL,
    ...
    FOREIGN KEY (notif_usr_id) REFERENCES User(user_id) ON DELETE CASCADE,
    FOREIGN KEY (notif_item_id) REFERENCES item(item_id) ON DELETE CASCADE
);

CREATE TABLE Reply (
    reply_id INT AUTO_INCREMENT PRIMARY KEY,
    reply_item_id INT NOT NULL,
    reply_usr_id INT NOT NULL,
    reply_text VARCHAR(500) NOT NULL,
    reply_created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (reply_item_id) REFERENCES item(item_id) ON DELETE CASCADE,
    FOREIGN KEY (reply_usr_id) REFERENCES User(user_id) ON DELETE CASCADE
);
...
CREATE TABLE Push_Subscription (
    ...
    sub_endpoint VARCHAR(500) NOT NULL UNIQUE,
    sub_p256dh VARCHAR(200) NOT NULL,
    sub_auth VARCHAR(200) NOT NULL,
    FOREIGN KEY (sub_usr_id) REFERENCES User(user_id) ON DELETE CASCADE
);